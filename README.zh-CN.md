...
<p align="center">
  <img src="docs/screenshots/today-dark.zh.png" alt="桌面端今日一期：带战术标尺的日期标题、玻璃面板上的今日要点与先看重点、筛选栏和带准星框的榜单网格（深色）" width="100%">
</p>
...
- **Titanium × Signal 视觉（战术玻璃）。** 带顶光、边缘折射和投影厚度的毛玻璃面板，叠在会发光的战术网格上；准星框悬停锁定，深色是琥珀辉光，浅色是工业灰白配安全黄；面板“上电”入场、换页扫描线（尊重减少动效设置），深浅色跟随系统。另有 `paper`、`terminal`
