...
<p align="center">
  ...
  每天 5 × 10 条 AI 精选：开源项目、论文、Hacker News、X/Reddit、大厂动态。<br>
  透明评分、跨源共振、半年记忆，零服务器部署。
</p>
...
<p align="center">
  <a href="https://wzznne.github.io/AI-Resonance/">在线演示</a> ·
  <a href="#五分钟上手">快速上手</a> ·
  <a href="#功能一览">功能</a> ·
...
<p align="center">
  <img src="docs/screenshots/today-dark.zh.png" alt="桌面端今日一期：日期标题、共振簇、分类筛选栏和榜单网格（深色）" width="100%">
</p>

> 下文链接的 `docs/` 文档目前只有英文版；这一页已经涵盖上手所需的全部要点。
...
- **跨源共振。** 同一个项目、同一篇论文、同一次发布，同一天出现在 GitHub、arXiv、Hacker News、X/Reddit 和官方博客上，
  这些条目会被串成一个“共振簇”，放在页面最前面。多个来源不约而同地关注同一件事，就是最强的信号。
- **半年记忆。** 每个条目都保留 183 天的历史：首次出现、上榜天数、连续上榜、最佳名次、`NEW` / `▲3` / `▼1` / `BACK`
  标记和迷你走势图。一眼就能分清是昙花一现还是持续走热。

整个项目只跑在 GitHub Actions 和 GitHub Pages 上，没有后端、没有数据库、不需要注册账号。AI 功能在你的浏览器里用你自己的
Key 运行。改一个 YAML 文件，同一套代码就能追踪 Rust、机器人或任何你关心的主题。
...
- **Titanium × Signal 视觉。** 有凹凸质感的面板与凹槽，一缕克制的青紫“信号光”，深浅色跟随系统。另有 `paper`、`terminal`
...
- **中英双语。** 界面支持中文和英文。配置了流水线 LLM Key 后，简介、“为什么值得关注”、精华要点和每日要点也都有中英两个版本，
  还能一键切回原文。
...
- **订阅源**：中英两套 Atom 订阅源和 Markdown 摘要。
...
**是 LLM 在挑内容吗？** 不是。排名完全是对公开信号做算术。配置了 Key 时，LLM 只负责写简介和今日要点；要点里的每一条都必须
注明出处 `榜单#名次`，出处对不上的条目会被直接丢掉。
